...
export class MorseClockComponent implements OnDestroy
{
    ...
    @Input() set second(v: string) { if (v !== this._second) { this._second = v; this.resetSecond(); } }
    ...
    private T_S: MorseTiming = {dot: 60, dash: 180, intra: 60, letter: 180, loop: 20};
    ...
    private resetSecond() {this.buildAndPlay(this._second, this.blinkSSetter, this.timerSetterS, this.T_S);}

    private blinkHSetter = (v: boolean) => this.blinkH = v;
    private blinkMSetter = (v: boolean) => this.blinkM = v;
    private blinkSSetter = (v: boolean) => this.blinkS = v;

    private timerSetterH = (h: any) => this.timerH = h;
    private timerSetterM = (m: any) => this.timerM = m;
    private timerSetterS = (s: any) => this.timerS = s;
    ...
    private buildAndPlay(val: string, ledSetter: (on: boolean) => void, timerSetter: (t: any) => void, T: MorseTiming)
    {
        clearTimeout(timerSetter === this.timerSetterH ?
            this.timerH : timerSetter === this.timerSetterM ? this.timerM : this.timerS);
